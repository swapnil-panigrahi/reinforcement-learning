...
def multiArmedBandits(k, steps, epsilon):
    phiStar = np.zeros(k)
    for _ in range(k):
...
def averageRuns(k, steps, epsilon, runs):
    averageRewards = np.zeros(steps)
    optimalActions = np.zeros(steps)
    for i in range(runs):
        x, y = multiArmedBandits(k, steps, epsilon)
        averageRewards += x
        optimalActions += y
    
    return averageRewards/runs, optimalActions/runs
...
for eps in epsilon:
    x, y = averageRuns(k, steps, eps, runs)
    avgRewards.append(x)
    optimalAction.append(y)    
    
x, y = averageRuns(k, steps, decay, runs)
avgRewards.append(x)
optimalAction.append(y)
...
plt.subplot(2, 1, 1)
for i, eps in enumerate(epsilon + ["1/t"]):
